...
dataroot=r"./dataset/merged_cicids2017.csv"  
...
# dataroot = '../dataset/merged_cicids2017.csv'
# df=pd.read_csv(dataroot,header=0,low_memory=False)
# from sklearn import preprocessing

# scaler = preprocessing.minmax_scale()
# df.iloc[:,0:-1] = scaler.fit_transform(df.iloc[:,0:-1])
# x_train,x_test,y_train,y_test = train_test_split(df.iloc[:,0:-1].as_matrix(), df['Label'].as_matrix(),test_size = 0.3, randome_state = 42)
# train_data = CustomData(x_train,x_test)
# test_data = CustomData(y_train,y_test)
